<p align="center">
  <img src="public/images/gigawallet-logo.png" alt="GigaWallet" width="280" />
</p>

# GigaWallet Documentation

This repository contains the documentation for [GigaWallet](https://gigawallet.dogecoin.org) — a backend for your Dogecoin business.

The site is built with [Fumadocs](https://www.fumadocs.dev/) (Next.js + MDX), with a flat structure at the repo root.
...
Static output is in `out/` (for GitHub Pages). The deploy workflow builds a static export and deploys to GitHub Pages.
...
git clone https://github.com/dogeorg/gigawallet-docs.git
cd gigawallet-docs
```
...
From the repo root (`gigawallet-docs`):
...
- **Edit or add doc pages:** MDX files in `content/docs/`, e.g. `content/docs/installation.mdx`.
- **Change sidebar order or labels:** Edit `content/docs/meta.json`.
- **Add or replace images:** Put files in `public/images/` and reference them in MDX as `/images/your-image.png`.
...
If the build finishes without errors, your changes are fine for production. The static site is in the `out/` directory.

### 7. Submit your improvements

1. **Fork** the repo on GitHub (click **Fork** on the [gigawallet-docs](https://github.com/dogeorg/gigawallet-docs) page).
2. **Create a branch** in your fork (e.g. `git checkout -b docs-fix-typo`).
3. **Commit** your changes (e.g. `git add .` then `git commit -m "Fix typo in installation"`).
...
| Images | `public/images/` (use in MDX as `/images/filename.png`) |
...
- **Deploy** (`.github/workflows/deploy.yml`): on push to `main`, builds a static export and deploys to GitHub Pages.

To publish via GitHub Pages: go to the repo **Settings → Pages**, set **Source** to **GitHub Actions**. After the next push to `main`, the site will be built and deployed. With a `CNAME` file (e.g. `gigawallet.dogecoin.org`), Pages will serve the site on that domain.

## Links

- **Live docs:** [gigawallet.dogecoin.org](https://gigawallet.dogecoin.org)
- **GigaWallet repo:** [github.com/dogecoinfoundation/gigawallet](https://github.com/dogecoinfoundation/gigawallet)
